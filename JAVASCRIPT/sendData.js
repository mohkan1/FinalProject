!function e(t, n, r) {
    ...
    2: [function(require, module, exports) {
        ...
        document.querySelector("form").addEventListener("submit", function(event) {
            function upload() {
                num >= 5 ? (num = 0, alert("https://mohkan1.github.io/projects/" + fileName), clearInterval(myVar)) : uploadFiles(fileName, editor.getValue(), commitTitle).then(function() {
                    num++
                }).catch(function(err) {
                    console.error(err), alert("Something went wrong. Please, try again."), num++
                })
            }
            event.preventDefault();
            var num = 0,
                commitTitle = document.getElementById("commit-title").value,
                fileName = "hunter/" + document.getElementById("fileName").value,
                procent = ["20%", "40%", "60%", "80%", "100%"],
                myVar = setInterval(upload, 2e3)
        })
    }, {
        "./github": 1
    }],
    ...
    4: [function(require, module, exports) {
        ! function(root, factory) {
            "use strict";
            if ("function" == typeof define && define.amd) define(["xmlhttprequest", "js-base64"], function(XMLHttpRequest, b64encode) {
                return root.Github = factory(XMLHttpRequest.XMLHttpRequest, b64encode.Base64.encode)
            });
            else if ("object" == typeof module && module.exports) "undefined" != typeof window ? module.exports = factory(window.XMLHttpRequest, window.btoa) : module.exports = factory(require("xmlhttprequest").XMLHttpRequest, require("js-base64").Base64.encode);
            else {
            ...
            var Github = function(options) {
                var API_URL = options.apiUrl || "https://api.github.com",
                    _request = Github._request = function(method, path, data, cb, raw, sync) {
                        var xhr = new XMLHttpRequest;
                        if (xhr.open(method, function() {
                                var url = path.indexOf("//") >= 0 ? path : API_URL + path;
                                if (url += /\?/.test(url) ? "&" : "?", data && "object" == typeof data && ["GET", "HEAD", "DELETE"].indexOf(method) > -1)
                                    for (var param in data) data.hasOwnProperty(param) && (url += "&" + encodeURIComponent(param) + "=" + encodeURIComponent(data[param]));
                                return url + ("undefined" != typeof window ? "&" + (new Date).getTime() : "")
                            }(), !sync), sync || (xhr.onreadystatechange = function() {
                                4 === this.readyState && (this.status >= 200 && this.status < 300 || 304 === this.status ? cb(null, raw ? this.responseText : !this.responseText || JSON.parse(this.responseText), this) : cb({
                                    path: path,
                                    request: this,
                                    error: this.status
                                }))
                ...
                return Github.User = function() {
                    this.repos = function(options, cb) {
                        1 === arguments.length && "function" == typeof arguments[0] && (cb = options, options = {}), options = options || {};
                        var url = "/user/repos",
                            params = [];
                        params.push("type=" + encodeURIComponent(options.type || "all")), params.push("sort=" + encodeURIComponent(options.sort || "updated")), params.push("per_page=" + encodeURIComponent(options.per_page || "1000")), options.page && params.push("page=" + encodeURIComponent(options.page)), url += "?" + params.join("&"), _request("GET", url, null, cb)
                        ...
                        var url = "/notifications",
                            params = [];
                        if (options.all && params.push("all=true"), options.participating && params.push("participating=true"), options.since) {
                        ...
                        options.page && params.push("page=" + encodeURIComponent(options.page)), params.length > 0 && (url += "?" + params.join("&")), _request("GET", url, null, cb)
                    ...
                    var repoPath, repo = options.name,
                        user = options.user,
                    ...
                    this.deleteRepo = function(cb) {
                        ...
                        var url = repoPath + "/pulls",
                            params = [];
                        "string" == typeof options ? params.push("state=" + options) : (options.state && params.push("state=" + encodeURIComponent(options.state)), options.head && params.push("head=" + encodeURIComponent(options.head)), options.base && params.push("base=" + encodeURIComponent(options.base)), options.sort && params.push("sort=" + encodeURIComponent(options.sort)), options.direction && params.push("direction=" + encodeURIComponent(options.direction)), options.page && params.push("page=" + options.page), options.per_page && params.push("per_page=" + options.per_page)), params.length > 0 && (url += "?" + params.join("&")), _request("GET", url, null, function(err, pulls, xhr) {
                            if (err) return cb(err);
                            cb(null, pulls, xhr)
                        })
                        ...
                        void 0 === cb && (cb = options, options = {}), that.getSha(branch, encodeURI(path), function(err, sha) {
                            var writeOptions = {
                                message: message,
                                content: void 0 === options.encode || options.encode ? b64encode(content) : content,
                                branch: branch,
    ...
    5: [function(require, module, exports) {
        (function(global) {
            ! function(global, factory) {
                "object" == typeof exports && void 0 !== module ? module.exports = factory(global) : "function" == typeof define && define.amd ? define(factory) : factory(global)
                ...
                var b64chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/",
                    ...
                    _encode = buffer ? buffer.from && Uint8Array && buffer.from !== Uint8Array.from ? function(u) {
